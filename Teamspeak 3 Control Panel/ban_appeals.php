<?php
	...
	$ts3_VirtualServer = TeamSpeak3::factory("serverquery://USERNAME:PASSWORD@SERVER_IP:QUERY_PORT/?server_port=SERVER_PORT&nickname=Legacy"); // change the info to your own usage
...
<body class="bg">
  		...
  		<ul class="nav navbar-nav">
			...
			<ul class="nav navbar-nav navbar-right">
				...
				if ($_SESSION['access'] >= 9) {
					echo '<li class="active"><a href="#">Ban Appeals</a></li>';
				}
	...
	<div class="container">
		<div class="head">Ban Appeals</div>
		<div class="pan-body">
			<table class="table">
				<thead>
					<th>Name</th>
				...
				<tbody>
					<?php
						$sql = "SELECT * FROM appeal_pug ORDER BY id DESC";
						$run_sql = mysqli_query($conn,$sql);
						if (mysqli_num_rows($run_sql) !== 0) {
							while ($rows = mysqli_fetch_assoc($run_sql)) {
								echo '<tr>';
								echo '<td>'.$rows['name'].'</td>';
								echo '<td>'.$rows['ts3_uid'].'</td>';
								echo '<td>'.$rows['appeal_response'].'</td>';
								echo '</tr>';
							}
						} else {
							echo '<tr><td>No Appeals Yet</td><td></td><td></td></tr>';
						}
						?>
					</tbody>
				</table>
