<?php
	...
	function register($email, $passowrd, $ip, $references, $dbid) {
		$em = mysqli_real_escape_string($conn, escape_input($email));
		$pass = hash('sha512',$password);
		$addr = mysqli_real_escape_string($conn, $_SERVER['REMOTE_ADDR']); // I dont remember whether the connection comes first or not for mysqli_real_escape_string();
		$reference =  mysqli_real_escape_string($conn, escape_input($references));
		$cldbid =  mysqli_real_escape_string($conn, escape_input($dbid));
