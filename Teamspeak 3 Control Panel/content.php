<?php
	...
	/*
		This page covers announcement contents
	*/
?>
		...
		<div class="pan-body">
			<div class="rows">
				...
				if ($_SESSION['access'] > 4) {
					if (isset($_GET['id'])) {
						$sql = "SELECT * FROM staff_announcement WHERE id='$_GET[id]'";
						$run_sql = mysqli_query($conn,$sql);
						if (mysqli_num_rows($run_sql) !== 0) {
							$row = mysqli_fetch_assoc($run_sql);
							echo '<div class="col-lg-4"><h2>'.$row['title'].'</h2> <h5 class="text-muted">Author: '.$row['author'].'</h5></div>';
							echo '<br><div class="col-lg-8"><p class="content">'.$row['content'].'</p></div>';
						}
					}
				} else {
